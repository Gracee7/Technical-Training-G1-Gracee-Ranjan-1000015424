...
try {
Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/userlogin", "root", "Aryan1001");
PreparedStatement ps = con.prepareStatement("insert into loginDetails values(?,?,?)");
PreparedStatement pst = con.prepareStatement("select * from loginDetails where Username='" + s2 + "'");
...
String sqlquery = "select * from logindetails where Username = '"+s2+"'";
